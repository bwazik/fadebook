...
<{{ $tag }}
    ...
    {{ $attributes->merge([
        'class' => "w-full liquid-glass liquid-button rounded-[1.5rem] p-5 flex items-center justify-between border-2 transition-all shadow-sm text-right group " .
        ($selected
            ? 'border-fadebook-accent bg-fadebook-accent/5'
...
>
    <div class="flex-1 pe-4">
        <div class="flex items-center gap-2">
            <h3 class="text-sm font-black text-gray-900 dark:text-white uppercase leading-tight group-hover:text-fadebook-accent transition-colors">
                {{ $service->name }}
            </h3>
        </div>
        
        @if($service->description)
            <p class="text-[10px] text-gray-400 font-bold mt-1 line-clamp-1 italic opacity-80">{{ $service->description }}</p>
        @endif
