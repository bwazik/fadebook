...
>
    <div class="flex justify-around items-center h-16 max-w-lg mx-auto px-2">
        ...
        {{-- Profile --}}
        <a href="{{ route('profile') }}" class="flex flex-col items-center gap-1 px-4 py-2 group">
            <svg class="w-6 h-6 transition-colors {{ request()->routeIs('profile*') ? 'text-[--color-fadebook-accent]' : 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-white/60' }}" fill="{{ request()->routeIs('profile*') ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/>
            </svg>
            <span class="text-[10px] font-medium {{ request()->routeIs('profile*') ? 'text-[--color-fadebook-accent]' : 'text-gray-400' }}">{{ __('messages.nav_profile') }}</span>
        </a>
