...
<div x-data="{
    current: localStorage.getItem('fadebook_accent') || '#ff2d55',
    setAccent(color) {
        this.current = color;
    ...
    <div class="flex items-center gap-3 justify-center py-2">
        @foreach($palettes as $key => $palette)
            <button
                type="button"
                @click="setAccent('{{ $palette['color'] }}')"
                title="{{ $palette['label'] }}"
                class="w-9 h-9 rounded-full transition-all duration-200 active:scale-90 relative"
                style="background-color: {{ $palette['color'] }};"
            >
                <div
                    x-show="current === '{{ $palette['color'] }}'"
                    class="absolute inset-x-[-4px] inset-y-[-4px] rounded-full border-2"
                    style="border-color: {{ $palette['color'] }};"
                ></div>
                <svg x-show="current === '{{ $palette['color'] }}'"
