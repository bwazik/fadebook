@props([
    ...
    'title',
    'subtitle' => null,
    'color' => 'fadebook-accent', // amber, red, fadebook-accent
    'dismissible' => false,
...
@php
    $colorMap = [
        'fadebook-accent' => [
            'border' => 'border-fadebook-accent/20',
            'bg' => 'bg-fadebook-accent/5',
            'iconBg' => 'bg-fadebook-accent',
            'iconText' => 'text-white',
            'iconShadow' => 'shadow-fadebook-accent/20',
        ],
        'amber' => [
    ...
    $colors = $colorMap[$color] ?? $colorMap['fadebook-accent'];
@endphp
...
<div {{ $attributes->merge(['class' => 'px-4']) }}>
    @if ($href)
        <a href="{{ $href }}" wire:navigate
            ...
            <div class="w-8 h-8 rounded-full {{ $colors['iconBg'] }} {{ $colors['iconText'] }} flex items-center justify-center shadow-sm">
                {{ $icon }}
            </div>
            <div class="flex-1">
    ...
    @else
        <div class="w-full liquid-glass rounded-2xl p-3 flex items-center gap-3 border {{ $colors['border'] }} {{ $colors['bg'] }} relative">
            <div class="w-8 h-8 rounded-full {{ $colors['iconBg'] }} {{ $colors['iconText'] }} flex items-center justify-center shadow-sm">
                {{ $icon }}
            </div>
            <div class="flex-1">
