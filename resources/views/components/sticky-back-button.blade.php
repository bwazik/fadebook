...
<div class="sticky top-0 z-50 h-0 pointer-events-none" x-data="{ scrolled: false }"
    ...
    <div class="flex justify-end px-2 pointer-events-auto transition-all duration-300"
        ...
        @if ($attributes->has('wire:click'))
            <button type="button"
                class="inline-flex items-center justify-center w-11 h-11 rounded-full liquid-glass !bg-black/20 !border-white/20 text-white shadow-xl transition-all active:scale-90 liquid-button pointer-events-auto"
                {{ $attributes }}>
