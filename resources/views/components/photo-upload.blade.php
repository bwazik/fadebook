...
{{-- Usage: <x-photo-upload :user="$user" wire-model="photo" /> --}}
...
<div>
    {{-- Clickable Avatar --}}
    <div class="relative {{ $sizeClasses }} rounded-full overflow-hidden
                border-2 border-black/5 dark:border-white/10
                cursor-pointer group shrink-0 transition-all duration-200"
        ...
        @else
            <img src="{{ $user->image_path
                ? (Str::startsWith($user->image_path, 'http')
                    ? $user->image_path
