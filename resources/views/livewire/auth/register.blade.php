            ...
            <div x-show="step === 2" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" style="display: none;">
                <div class="space-y-4 mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-6">
                        <label class="relative flex flex-col items-center p-4 bg-white/70 dark:bg-[#1c1c1e]/70 backdrop-blur-3xl border rounded-2xl cursor-pointer transition-all duration-200 text-center" :class="role === 'client' ? 'border-fadebook-accent shadow-sm ring-1 ring-fadebook-accent' : 'border-black/5 dark:border-white/10 hover:bg-white/80 dark:hover:bg-[#1c1c1e]/80'">
                            ...
                            <span class="block text-[10px] text-gray-500 dark:text-gray-400 mt-1">{{ __('messages.role_client_desc') }}</span>
                        </label>

                        <label class="relative flex flex-col items-center p-4 bg-white/70 dark:bg-[#1c1c1e]/70 backdrop-blur-3xl border rounded-2xl cursor-pointer transition-all duration-200 text-center" :class="role === 'barber_owner' ? 'border-fadebook-accent shadow-sm ring-1 ring-fadebook-accent' : 'border-black/5 dark:border-white/10 hover:bg-white/80 dark:hover:bg-[#1c1c1e]/80'">
                            ...
                            <span class="block text-[10px] text-gray-500 dark:text-gray-400 mt-1">{{ __('messages.role_barber_desc') }}</span>
                        </label>
                    </div>

                    <!-- Shop Info Fields (Shown only for Barber) -->
                    <div x-show="role === 'barber_owner'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="space-y-4 mt-6 pt-6 border-t border-black/5 dark:border-white/10">
                        <p class="text-xs font-black text-fadebook-accent uppercase tracking-widest text-center mb-4">{{ __('messages.register_shop_header') }}</p>
                        
                        <x-ios-input label="{{ __('messages.register_shop_name_label') }}" wire:model="shopName" type="text" placeholder="{{ __('messages.register_shop_name_placeholder') }}" />
                        
                        <x-ios-input label="{{ __('messages.register_shop_phone_label') }}" wire:model="shopPhone" type="tel" dir="ltr" placeholder="01xxxxxxxxx" />
                        
                        <x-ios-select label="{{ __('messages.register_area_label') }}" wire:model="areaId" :options="$areasOptions" placeholder="{{ __('messages.register_area_placeholder') }}" class="w-full" />

                        <x-ios-input label="{{ __('messages.register_address_label') }}" wire:model="address" type="text" placeholder="{{ __('messages.register_address_placeholder') }}" />
                    </div>
                </div>

                <div class="flex gap-3">
                    <div class="w-1/3">
                        <x-ios-button type="button" wire:click="previousStep" variant="secondary">{{ __('messages.back') }}</x-ios-button>
                    </div>
                    <div class="w-2/3">
                        <x-ios-button type="submit" target="register">{{ __('messages.create_account') }}</x-ios-button>
                    </div>
                </div>
            </div>
        </form>
