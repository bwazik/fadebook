        ...
        @if($step === 1)
            ...
            <form wire:submit="verifyPasswordAndSendOtp" class="space-y-4">
                <div class="mb-4 space-y-4">
                    <x-ios-input label="{{ __('messages.current_password') }}" wire:model="current_password" type="password" placeholder="{{ __('messages.current_password') }}" :disabled="! $canChange" dir="ltr" />
                    <x-ios-input label="{{ __('messages.new_phone') }}" wire:model="new_phone" type="tel" placeholder="01xxxxxxxxx" dir="ltr" :disabled="! $canChange" />
                </div>

                <x-ios-button type="submit" target="verifyPasswordAndSendOtp" :disabled="! $canChange">{{ __('messages.send_verification_code') }}</x-ios-button>
            </form>
        @endif
        ...
        @if($step === 2)
            ...
            <form wire:submit="verifyOtp" class="space-y-4">
                ...
                <div class="pt-2 space-y-3">
                    ...
                    <button type="button" wire:click="goBack" class="w-full bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold py-3 px-4 rounded-2xl transition duration-200 cursor-pointer">
            ...
            <div class="mt-6 text-sm text-gray-500 font-tajawal text-center">
                ...
                <button type="button" wire:click="resendOtp" wire:loading.attr="disabled" class="text-fadebook-accent hover:text-red-600 font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
