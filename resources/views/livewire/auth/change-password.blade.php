<div class="pt-16 pb-8 px-2 w-full" x-data="{
    ...
    <div class="w-full">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">{{ __('messages.change_password_title') }}
            </h1>
        ...
        <div x-show="step === 1" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="space-y-6">
                <p class="text-sm text-gray-600 dark:text-gray-400 text-center">
                    {!! __('messages.change_password_confirmation_hint', [
                        'phone' => '<span dir="ltr" class="inline-block font-bold">' . $maskedPhone . '</span>',
                    ]) !!}
                </p>

                <div class="mb-4 space-y-4">
                    <x-ios-input label="{{ __('messages.current_password') }}" wire:model="current_password"
                        type="password" id="current_password" dir="ltr" placeholder="••••••••" />
                </div>

                <x-ios-button wire:click="sendOtp" type="button" target="sendOtp"
                    class="mt-0">{{ __('messages.send_verification_code') }}</x-ios-button>
            </div>
        </div>
        ...
        <div x-show="step === 2" x-transition:enter="transition ease-out duration-300"
            ...
            <form wire:submit="verifyOtp" class="space-y-4">
                <p class="text-sm text-center text-gray-600 dark:text-gray-400 mb-4">
                    {{ __('messages.otp_whatsapp_hint', ['phone' => $phone]) }}</p>
                <div class="mb-4">
