<div class="pb-[calc(5rem+var(--safe-area-bottom)+64px)] relative min-h-screen pt-4 px-4">
    ...
    <div class="space-y-8">
            ...
            <div>
                ...
                <div class="space-y-3" wire:sort="updateOrder">
                    @foreach ($services as $service)
                        <div wire:sort:item="{{ $service->id }}" wire:key="service-{{ $service->id }}"
                            class="liquid-glass rounded-[1.5rem] p-4 flex items-center justify-between border-white/30 dark:border-white/10 shadow-sm {{ !$service->is_active ? 'opacity-50 grayscale' : '' }}">
                            <div class="flex items-center gap-4 flex-1">
                                ...
                                <div class="flex-1 pe-4">
                                    ...
                                    @if ($service->description)
                                        <p
                                            class="text-[9px] text-gray-400 font-bold mt-1.5 italic line-clamp-1 opacity-80">
                                    ...
                                    <div class="flex items-center gap-2 mt-2.5">
                                        <div
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                ...
                                                class="w-2.5 h-2.5 text-banhafade-accent">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div class="shrink-0 flex items-center gap-3">
                                <div class="text-left me-2">
                                    <p class="text-lg font-black text-banhafade-accent tracking-tighter leading-none">
        ...
        @empty
            <x-empty-state title="{{ __('messages.no_services') }}"
    ...
    <x-bottom-sheet wire:model="showForm" :title="$editingId ? __('messages.edit_service') : __('messages.add_new_service')">
        <form wire:submit="save" class="space-y-6 pb-6 pt-2">
            <x-ios-input label="{{ __('messages.service_name_label') }}" wire:model="name" type="text"
                placeholder="{{ __('messages.service_name_placeholder') }}" />

            <x-ios-textarea label="{{ __('messages.service_desc_label') }}" wire:model="description"
                placeholder="{{ __('messages.service_desc_placeholder') }}" />

            <div class="grid grid-cols-2 gap-4">
                <x-ios-input label="{{ __('messages.price_label') }}" wire:model="price" type="number"
                    dir="ltr" placeholder="0" />
                <x-ios-input label="{{ __('messages.duration_label') }}" wire:model="duration_minutes"
                    type="number" dir="ltr" placeholder="30" />
            </div>

            <x-ios-select label="{{ __('messages.category_label') }}" wire:model="service_category_id"
                :options="$this->categories->pluck('name', 'id')->toArray()" placeholder="{{ __('messages.other_category') }}" />
