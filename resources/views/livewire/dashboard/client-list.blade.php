<div class="pb-[calc(5rem+var(--safe-area-bottom)+64px)] relative min-h-screen pt-4 px-4">
    ...
    <div class="mb-6 relative">
        ...
        <input type="text" wire:model.live.debounce.300ms="search"
            class="block w-full py-3 pr-11 pl-4 text-gray-900 bg-white border border-gray-200 rounded-[1.2rem] text-sm focus:ring-banhafade-accent focus:border-banhafade-accent dark:bg-[#1c1c1e] dark:border-white/10 dark:placeholder-gray-500 dark:text-white text-right"
        ...
        @forelse($this->clients as $client)
            <div class="liquid-glass rounded-[1.5rem] p-4 border border-white/20 shadow-sm flex items-center gap-4">
                ...
                <div class="shrink-0 text-center">
                    <span
                        class="block text-xl font-black text-banhafade-accent leading-none">{{ $client->total_visits }}</span>
                    <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">زيارة</span>
                </div>
            </div>
        @empty
            <x-empty-state title="لا يوجد عملاء" description="لم يقم أي عميل بالحجز في محلك حتى الآن.">
                <x-slot name="icon">
