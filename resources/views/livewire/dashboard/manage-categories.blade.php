<div class="pb-[calc(5rem+var(--safe-area-bottom)+64px)] relative min-h-screen pt-4 px-4">
    ...
    <!-- Alert for info -->
    <div class="mb-6 px-2">
        <div class="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/20 rounded-[1.2rem] p-4 flex gap-3">
    ...
    >
        @forelse($this->categories as $category)
            <div data-id="{{ $category->id }}" class="liquid-glass rounded-[1.5rem] p-4 flex items-center justify-between border-white/30 dark:border-white/10 shadow-sm">
                ...
                <div class="flex items-center gap-1.5">
                    <button wire:click="edit({{ $category->id }})" class="p-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 active:scale-95 transition-transform cursor-pointer">
        ...
        @empty
            <x-empty-state 
                title="{{ __('messages.no_categories') }}"
    ...
    <x-bottom-sheet wire:model="showForm" :title="$editingId ? __('messages.edit_category') : __('messages.add_new_category')">
        <form wire:submit="save" class="space-y-6 pb-6 pt-2">
            <x-ios-input label="{{ __('messages.category_name_label') }}" wire:model="name" type="text" placeholder="{{ __('messages.category_name_placeholder') }}" />
