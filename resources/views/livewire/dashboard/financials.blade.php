<div class="pb-[calc(5rem+var(--safe-area-bottom)+64px)] relative min-h-screen pt-4 px-4">
    ...
    <div class="space-y-4">
        <div class="liquid-glass rounded-2xl p-6 border border-white/20 shadow-sm relative overflow-hidden">
            ...
            <p class="text-xs font-black text-gray-500 uppercase tracking-widest mb-2 relative z-10">{{ __('messages.gross_earnings') }}</p>
            <p class="text-4xl font-black text-fadebook-accent leading-none relative z-10">{{ number_format($this->stats['gross_earnings'], 0) }} <span class="text-sm">{{ __('messages.egp') }}</span></p>
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div class="liquid-glass rounded-2xl p-5 border border-white/20 shadow-sm">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">{{ __('messages.commission_deducted') }} ({{ $this->stats['commission_rate'] }}%)</p>
                <p class="text-xl font-black text-red-500 dark:text-red-400 leading-none">-{{ number_format($this->stats['commission_deducted'], 0) }} <span class="text-[10px]">{{ __('messages.egp') }}</span></p>
            </div>
            <div class="liquid-glass rounded-2xl p-5 border border-white/20 shadow-sm">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">{{ __('messages.net_payout') }}</p>
                <p class="text-xl font-black text-green-500 dark:text-green-400 leading-none">{{ number_format($this->stats['net_payout'], 0) }} <span class="text-[10px]">{{ __('messages.egp') }}</span></p>
            </div>
        </div>

        <div class="liquid-glass rounded-2xl p-5 border border-white/20 shadow-sm flex items-center justify-between mt-4">
            <div>
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">{{ __('messages.total_bookings_completed') }}</p>
                <p class="text-lg font-black text-gray-900 dark:text-white leading-none">{{ $this->stats['total_bookings'] }} <span class="text-[10px]">{{ __('messages.booking_unit') }}</span></p>
            </div>
            <div class="w-12 h-12 rounded-full bg-fadebook-accent/10 flex items-center justify-center text-fadebook-accent">
    ...
    <!-- Transactions List -->
    <div class="mt-8">
        <h2 class="text-xs font-black text-gray-500 uppercase tracking-[0.2em] mb-4">{{ __('messages.transactions_list_title') }}</h2>
        
        <div class="space-y-3">
            @forelse($this->transactions as $tx)
                <div class="liquid-glass rounded-2xl p-4 border border-white/20 shadow-sm flex items-center justify-between relative active:scale-[0.98] transition-transform">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-fadebook-accent/5 flex items-center justify-center text-fadebook-accent">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182 1.128-.879 2.97-.879 4.098 0 .144.113.27.238.38.373" />
                            </svg>
                        </div>
                        <div>
                    ...
                    <div class="text-left">
                        <p class="text-sm font-black text-gray-900 dark:text-white leading-none">
                            {{ number_format($tx->final_amount, 0) }} <span class="text-[9px]">{{ __('messages.egp') }}</span>
                        </p>
                        @php
                            $rate = $tx->shop->commission_rate ?? 10.00;
