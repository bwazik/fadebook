<div class="pb-[calc(5rem+var(--safe-area-bottom)+64px)] min-h-screen pt-4 px-4 relative">
    ...
    <x-ios-input-group class="mt-4">
        @forelse($notifications as $notification)
            <div wire:key="notification-{{ $notification->id }}"
                wire:click="handleNotificationClick('{{ $notification->id }}')" @class([
                ...
                <div class="relative shrink-0 pt-0.5">
                    <div @class([
                        ...
                        $notification->data['icon_bg'] ?? '' =>
                            !$notification->read_at && isset($notification->data['icon_bg']),
                    ])>
                        @php
        ...
        @empty
            <div class="p-12 text-center">
                <div
                    class="w-20 h-20 bg-gray-50 dark:bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4 border border-black/5 dark:border-white/10">
                ...
                <h3 class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tighter">
                    {{ __('لا يوجد إشعارات') }}</h3>
                <p class="text-[11px] font-bold text-gray-400 mt-2">
                    {{ __('سوف تظهر هنا التنبيهات المتعلقة بحسابك وحجوزاتك.') }}</p>
            </div>
        @endforelse
    </x-ios-input-group>
    ...
    <div class="mt-6 px-4">
        @if ($this->hasMore)
            <div wire:key="sentinel-{{ $this->perPage }}" wire:intersect="loadMore" class="flex justify-center py-6">
                <div
                    class="flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/40 dark:bg-white/5 backdrop-blur-xl border border-black/5 dark:border-white/10 shadow-sm animate-pulse">
                    <span
                        class="text-[10px] font-black text-gray-500 uppercase tracking-widest">{{ __('جاري التحميل...') }}</span>
                    <svg class="animate-spin h-3.5 w-3.5 text-banhafade-accent" xmlns="http://www.w3.org/2000/svg"
        ...
        @elseif($notifications->count() > 0)
            <div wire:key="sentinel-end" class="flex justify-center py-8">
                <div class="flex flex-col items-center gap-2">
                    ...
                    <p class="text-[9px] text-gray-400 font-black uppercase tracking-[0.2em]">{{ __('نهاية القائمة') }}
                    </p>
                </div>
            </div>
        @endif
