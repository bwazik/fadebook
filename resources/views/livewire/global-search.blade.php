        ...
        <div class="px-1 pb-8">
            ...
            <div class="relative mb-6">
                ...
                <input type="text" wire:model.live.debounce.300ms="query"
                    ...
                    class="w-full bg-black/5 dark:bg-white/5 border-2 border-transparent focus:border-banhafade-accent/30 rounded-2xl py-4 pr-12 pl-4 text-sm font-bold text-gray-900 dark:text-white outline-none transition-all placeholder:text-gray-400"
                    x-ref="searchInput" @opened.window="setTimeout(() => $refs.searchInput.focus(), 100)">
            </div>

            {{-- 1. Loader Overlay --}}
            <div wire:loading wire:target="query"
                class="w-full min-h-[300px] grid place-items-center text-center content-center">
                <div class="flex flex-col items-center">
                    <div
                        class="w-10 h-10 border-4 border-banhafade-accent/20 border-t-banhafade-accent rounded-full animate-spin mb-4">
                    </div>
                    <p class="text-[10px] text-gray-500 font-bold uppercase tracking-widest">
                        {{ __('messages.searching_now') }}</p>
                </div>
            </div>
            ...
            <div wire:loading.remove wire:target="query">
                ...
                @if (empty($query))
                            ...
                            <div class="space-y-3">
                                @foreach ($this->suggestions as $suggestion)
                                    <a href="{{ $context === 'dashboard' ? route('dashboard.reservations', ['search' => $suggestion->booking_code]) : route('booking.show', $suggestion->uuid) }}"
                                        ...
                                        <div
                                            class="w-10 h-10 rounded-xl bg-banhafade-accent/10 flex items-center justify-center shrink-0">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="2" stroke="currentColor"
                                                class="w-5 h-5 text-banhafade-accent">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center justify-between gap-2">
                                                ...
                                                <span
                                                    class="text-[9px] font-bold text-gray-400">{{ \Carbon\Carbon::parse($suggestion->scheduled_at)->format('H:i') }}</span>
                                            </div>
                                            <p class="text-[10px] text-gray-500 font-bold truncate uppercase mt-0.5">
                                                {{ $suggestion->service?->name ?? 'خدمة' }} •
                                                #{{ $suggestion->booking_code }}
                                            </p>
                                        </div>
                                    </a>
                                @endforeach
                    ...
                    @else
                        ...
                        <x-empty-state class="min-h-[300px]" title="{{ __('messages.start_typing_to_search') }}"
                    ...
                    <div class="space-y-2">
                        @forelse($this->results as $result)
                            <a href="{{ $result['url'] }}" wire:navigate
                                ...
                                <div
                                    class="w-12 h-12 rounded-full overflow-hidden bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 shrink-0">
                                    @if ($result['image'])
                                        <img src="{{ Storage::url($result['image']) }}"
                                            class="w-full h-full object-cover">
                                    @else
                                        <div
                                            class="w-full h-full flex items-center justify-center text-gray-400 font-black text-sm uppercase">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4
                                        class="text-sm font-black text-gray-900 dark:text-white truncate group-hover:text-banhafade-accent transition-colors">
                                        {{ $result['title'] }}
                                    </h4>
                        ...
                        @empty
                            ...
                            <x-empty-state class="min-h-[300px]" title="{{ __('messages.no_results_found') }}"
